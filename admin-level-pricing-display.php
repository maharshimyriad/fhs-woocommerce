...
function fhs_get_level_a_sale_price( $product ) {
	...
	return get_post_meta( $product->get_id(), '_LevelA_tiered_price_sale_price', true );
}
...
function fhs_display_level_a_column( $column, $post_id ) {
	if ( 'level_a_price' === $column ) {
		...
		$sale_price = get_post_meta( $post_id, '_LevelA_tiered_price_sale_price', true );

		if ( $regular_price ) {
			if ( $sale_price && $sale_price < (float) $regular_price ) {
				echo wp_kses_post( wc_format_sale_price( $regular_price, $sale_price ) );
			} else {
