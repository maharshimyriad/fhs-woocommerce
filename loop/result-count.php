...
<p class="woocommerce-result-count" <?php echo ( empty( $orderedby ) || 1 === intval( $total ) ) ? '' : 'role="alert" aria-relevant="all" data-is-sorted-by="true"'; ?>>
	...
	if ( 1 === intval( $total ) ) {
		...
		printf( _n( '%1$d product', '%1$d products', $total, 'woocommerce' ) . $orderedby_placeholder, $total, esc_html( $orderedby ) );
	} else {
		...
		printf( _nx( 'Showing %1$d&ndash;%2$d of %3$d product', 'Showing %1$d&ndash;%2$d of %3$d products', $total, 'with first and last result', 'woocommerce' ) . $orderedby_placeholder, $first, $last, $total, esc_html( $orderedby ) );
	}
