...
<?php if ( $has_orders ) : ?>
<?php 
$order_tabs_new = wc_get_account_orders_columns();
if (isset($order_tabs_new['reorder'])) { // Adjust 'reorder' to the actual key
        unset($order_tabs_new['reorder']);
    }
?>
	<table class="woocommerce-orders-table woocommerce-MyAccount-orders shop_table shop_table_responsive my_account_orders account-orders-table">
		<thead>
			<tr>
				<?php foreach ( $order_tabs_new as $column_id => $column_name ) : ?>
					...
					?>
						...
						<?php endif; ?>
							...
							<?php elseif ( 'order-actions' === $column_id ) : ?>
								...
								echo "<div class='order-action-container'>";
								if ( ! empty( $actions ) ) {
									foreach ( $actions as $key => $action ) { // phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited
                                        ...
                                        echo ($key == 'view') ? '<i class="icofont-eye-alt"></i>' : '<img src="https://fhs.com.au/wp-content/uploads/2025/04/Mask-Group-23.svg" alt="Action Icon">';
                                        echo '</a>';
